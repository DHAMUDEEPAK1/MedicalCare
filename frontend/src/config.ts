...
const DEFAULT_STORAGE_GATEWAY_URL = 'https://blob.medicalcare';
...
export async function createActorWithConfig(options?: CreateActorOptions): Promise<backendInterface> {
    ...
    // Storage client and gateway dependencies removed since storage is on-canister now.

    const uploadFile = async (file: ExternalBlob): Promise<Uint8Array> => {
        // Bypass storage gateway to fix upload failures, store bytes directly on the backend canister
        return new Uint8Array(await file.getBytes());
    };

    const downloadFile = async (bytes: Uint8Array): Promise<ExternalBlob> => {
        // The bytes stored in motoko are the actual file bytes directly
